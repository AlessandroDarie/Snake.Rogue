...
GAME_HEIGHT = HEIGHT  # Rimuovo la sezione SCORE_HEIGHT
...
def message(msg, color, position, font=font_style):
...
def gameMenu():
    menu = True
    selected_option = 0
    options = ["Play", "Difficulty", "Quit"]

    while menu:
        screen.fill(BLACK)
        message("Snake Game", YELLOW, [WIDTH / 2 - 100, HEIGHT / 5], font=title_font)
...
def changeDifficulty():
    ...
    while difficulty_menu:
        screen.fill(BLACK)
        message("Select Difficulty", YELLOW, [WIDTH / 2 - 150, HEIGHT / 5], font=title_font)
...
def pause_menu():
    ...
    while paused:
        screen.fill(BLACK)
        message("Paused", YELLOW, [WIDTH / 2 - 50, HEIGHT / 4], font=title_font)
...
def gameLoop():
    ...
    while not game_over:
        ...
        while game_close:
            screen.fill(BLACK)
            message("Game Over", YELLOW, [WIDTH / 2 - 100, HEIGHT / 5], font=title_font)
            options = ["Play Again", "Main Menu"]
            selected_option = 0

            for i, option in enumerate(options):
                color = WHITE if i != selected_option else YELLOW
                message(option, color, [WIDTH / 2 - 100, HEIGHT / 3 + i * 50])
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    mouse_pos = pygame.mouse.get_pos()
                    for i, option in enumerate(options):
                        if WIDTH / 2 - 100 <= mouse_pos[0] <= WIDTH / 2 + 200 and HEIGHT / 3 + i * 50 <= mouse_pos[1] <= HEIGHT / 3 + i * 50 + 30:
